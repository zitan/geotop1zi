...
cnet->Conc=new_doublevector(cnet->r->nh);
cnet->Inmass=new_doublevector(cnet->r->nh);
...
sl->D50=new_doubletensor(Nl,Nr,Nc);
initialize_doubletensor(sl->D50,0.0);

sl->agg=new_doubletensor(Nl,Nr,Nc);
initialize_doubletensor(sl->agg,0.0);

sl->coh=new_doubletensor(Nl,Nr,Nc);
initialize_doubletensor(sl->coh,0.0);

sl->cohad=new_doublematrix(Nr,Nc);
initialize_doublematrix(sl->cohad,0.0);

sl->rho=new_doubletensor(Nl,Nr,Nc);
initialize_doubletensor(sl->rho,0.0);

sl->er_r=new_doubletensor(Nl,Nr,Nc);
initialize_doubletensor(sl->er_r,0.0);

sl->Tv=new_doublematrix(Nr,Nc);
initialize_doublematrix(sl->Tv,0.0);

sl->vs=new_doublematrix(Nr,Nc);	
initialize_doublematrix(sl->vs,0.0);

sl->ys=new_doublematrix(Nr,Nc);	
initialize_doublematrix(sl->vs,0.0);

sl->POC=new_doublematrix(Nr,Nc);	
initialize_doublematrix(sl->POC,0.0);	

sl->DOC=new_doublematrix(Nr,Nc);	
initialize_doublematrix(sl->DOC,0.0);

sl->EOC=new_doublematrix(Nr,Nc);	
initialize_doublematrix(sl->EOC,0.0);
			...
			for(i=1;i<=sl->pa->ndh;i++){
				if(sl->type->co[r][c]==i){
					...
					for(l=1;l<=Nl;l++){
						...
						sl->D50->co[l][r][c]=sl->pa->co[sy][jd50][l];
						//printf("D50[%d][%d][%d]=%f\n",l,r,c,sl->D50->co[l][r][c]);
		    
						sl->agg->co[l][r][c]=sl->pa->co[sy][jagg][l];
						sl->coh->co[l][r][c]=sl->pa->co[sy][jcoh][l];
						sl->rho->co[l][r][c]=sl->pa->co[sy][jrho][l];
						sl->er_r->co[l][r][c]=sl->pa->co[sy][jerr][l];
...
/* Initialization of wat->hrill (h of the surface that reaches the rill in mm):*/

/*

wat->hp=new_doublematrix(Nr,Nc);	
initialize_doublematrix(wat->hp,0.0);
wat->q_sup=new_doublematrix(Nr,Nc);	
initialize_doublematrix(wat->q_sup,0.0);
wat->q1=new_doublematrix(Nr,Nc);	
initialize_doublematrix(wat->q1,0.0);
wat->C=new_doublematrix(Nr,Nc);	
initialize_doublematrix(wat->C,0.0);

*/
wat->hrill=new_doublematrix(Nr,Nc);	
initialize_doublematrix(wat->hrill,0.0);
wat->Min=new_doublematrix(Nr,Nc);	
initialize_doublematrix(wat->Min,0.0);	
wat->dm=new_doublematrix(Nr,Nc);	
initialize_doublematrix(wat->dm,0.0);
wat->Mc=new_doublematrix(Nr,Nc);
initialize_doublematrix(wat->Mc,0.0);
wat->Mp=new_doublematrix(Nr,Nc);	
initialize_doublematrix(wat->Mp,0.0);
wat->Mn=new_doublematrix(Nr,Nc);	
initialize_doublematrix(wat->Mn,0.0);
/*
wat->Ds=new_doublematrix(Nr,Nc);	
initialize_doublematrix(wat->Ds,0.0);
wat->Ke=new_doublematrix(Nr,Nc);	
initialize_doublematrix(wat->Ke,0.0);
wat->Tc=new_doublematrix(Nr,Nc);	
initialize_doublematrix(wat->Tc,0.0);
wat->v=new_doublematrix(Nr,Nc);	
initialize_doublematrix(wat->v,0.0);
*/
...
/****************************************************************************************************/
//reading SOIL pocc MAP
sl->pocc=new_doublematrix(top->Z0->nrh,top->Z0->nch);	

if(existing_file(files->co[fpocc]+1)>0){
sl->pocc=read_map(2, files->co[fpocc]+1, land->LC, UV);
}else{//default value (0.035)
	for(r=1;r<=top->Z0->nrh;r++){
		for(c=1;c<=top->Z0->nch;c++){
			if(land->LC->co[r][c]!=UV->V->co[2]){
				sl->pocc->co[r][c] = 0.035;
			}else{sl->pocc->co[r][c] = UV->V->co[2];}
		}
	}
}
...
free_doublematrix(M);
/****************************************************************************************************/
//reading SOIL docc MAP
sl->docc=new_doublematrix(top->Z0->nrh,top->Z0->nch);

if(existing_file(files->co[fdocc]+1)>0){
	sl->docc=read_map(2, files->co[fdocc]+1, land->LC, UV);
...
void read_parameterfile(char *name, PAR *par, INIT_TOOLS *itools){
	...
	if(v->nh!=12) t_error("Block 2 of parameter file requires 12 parameters - Please review them");
...
void read_optionsfile_distr(char *name, PAR *par, TIMES *times, DOUBLEMATRIX *Z0){
	...
	par->output_Ero=v->co[19+i];
	par->output_Eoc=v->co[20+i];
	free_doublevector(v);
