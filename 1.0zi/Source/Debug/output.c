...
// static double Qsub_ch, Qsup_ch;
 static double Qsuptot; /*averaged surface output flows*/
 static double Qsubtot; /*averaged subsurface output flows*/
 static double Etot; /*averaged output soil erosion*/
 static double EOCtot; /*averaged output soil organic carbon loss*/
...
if(par->state_discharge==1 && strcmp(files->co[fQ]+1 , MISSING_FILE) != 0){
	...
	if(times->time==0) {Qsuptot=0.0; Qsubtot=0.0; Etot=0.0; EOCtot=0.0;}
	Qsuptot += cnet->Q_out;
	Qsubtot += cnet->Qsub_out;
	Etot +=sl->outero;
	EOCtot +=sl->OTEOC;
	...
	if (times->i_discharge==times->n_discharge){/*Print the outlet flows*/
		...
		fprintf(f,",%f,%f,%f,%f\n",Qsuptot/((double)times->n_discharge),Qsubtot/((double)times->n_discharge),Etot,EOCtot);
		fclose(f);
		free(name);
...
//OUT Erosion
if(par->output_Ero>0 && fmod(times->time+par->Dt,par->output_Ero*3600.0)<1.E-5){
	n_file=(long)((times->time+par->Dt)/(par->output_Ero*3600.0));	
	write_suffix(SSSS, n_file, 0);
		
	if(strcmp(files->co[fer]+1 , MISSING_FILE) != 0){
		name=join_strings(files->co[fer]+1,"tot");
		temp1=join_strings(name,SSSS);
		write_map(temp1, 0, par->format_out, wat->Mn, UV);
		//initmatrix(0.0, wat->Mn, land->LC, NoV);	
		free(temp1);
		free(name);	
    }  
}	

//OUT Organic carbon loss
if(par->output_Eoc>0 && fmod(times->time+par->Dt,par->output_Eoc*3600.0)<1.E-5){
	n_file=(long)((times->time+par->Dt)/(par->output_Eoc*3600.0));	
	write_suffix(SSSS, n_file, 0);
		
	if(strcmp(files->co[feoc]+1 , MISSING_FILE) != 0){
		name=join_strings(files->co[feoc]+1,"tot");
		temp1=join_strings(name,SSSS);
		write_map(temp1, 0, par->format_out, sl->EOC, UV);
		//initmatrix(0.0, sl->EOC, land->LC, NoV);	
		free(temp1);
		free(name);	
    }  
}		
...
// free_longvector(top->Ui);
 ...
 free_doubletensor(sl->ET);
	...
	free_doubletensor(sl->D50);
	free_doubletensor(sl->coh);
	free_doubletensor(sl->agg);
	free_doubletensor(sl->rho);
	free_doubletensor(sl->er_r);
	free_doublematrix(sl->ys);
	free_doublematrix(sl->vs);
	free_doublematrix(sl->POC);  
	free_doublematrix(sl->DOC);
	free_doublematrix(sl->EOC);     
	free_doublematrix(sl->pocc);
	free_doublematrix(sl->docc); 
 if(par->state_pixel==1){
 ...
 free_doublematrix(wat->hrill);
// free_doublematrix(wat->hp); 
// free_doublematrix(wat->q_sup);
// free_doublematrix(wat->q1);
 free_doublematrix(wat->dm);
// free_doublematrix(wat->C);
 //free_doublematrix(wat->Min);
 free_doublematrix(wat->Mc);
 free_doublematrix(wat->Mp);
 free_doublematrix(wat->Mn);
 //free_doublematrix(wat->Ds);
 //free_doublematrix(wat->Ke);
// free_doublematrix(wat->Tc);
// free_doublematrix(wat->v);
 ...
 free_doublevector(cnet->Conc);
 free_doublevector(cnet->Inmass);
